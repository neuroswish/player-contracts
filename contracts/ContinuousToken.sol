...
contract ContinuousToken is BondingCurve {
    ...
    function clearMatching() internal {
        ...
        if (resultOfSell >= cb.totalBuySpend) {
            ...
            uint256 remainingBuy = cb.totalBuySpend - resultOfSell;
            ...
            cb.poolBalance += remainingBuyReturn;
            cb.buysCleared = true;
        }
    }
    ...
    function claimBuy(uint256 batch, address sender) public {
        ...
        cb.buyers[sender] = 0;
        require(
            _burn(this, individualBuyReturn),
    ...
    function _mint(address minter, uint256 tokens) internal returns (bool) {
        supply += tokens;
        // TODO define balances mapping
        balances[minter] += tokens;
